  ...
  const parseTourData = (tour: any) => {
    ...
    try {
      ...
      return {
        ...
        isMultiDayTour: tour.isMultiDayTour || false,
        multiCityLocations: tour.multiCityLocations ? (typeof tour.multiCityLocations === 'string' ? JSON.parse(tour.multiCityLocations) : tour.multiCityLocations) : {} as Record<string, string[]>
      };
    } catch (e) {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (isEditing && submitForReview && tour?.status !== 'approved') {
          ...
          try {
          ...
          } catch (submitFetchError: unknown) {
            clearTimeout(submitTimeoutId);
            const sfErr = submitFetchError instanceof Error ? submitFetchError : new Error(String(submitFetchError));
            console.error('❌ Submit fetch error:', {
              name: sfErr.name,
              message: sfErr.message,
              error: submitFetchError
            });
            if (sfErr.name === 'AbortError') {
            ...
            if (sfErr.message?.includes('Failed to fetch') || sfErr.message?.includes('ERR_CONNECTION_REFUSED')) {
      ...
      if (err.message) {
        ...
        if (err.name === 'ReferenceError' && err.message.includes('supplier is not defined')) {
      ...
      const commonIssues = (error as Record<string, unknown>)?.commonIssues as string[] || [
