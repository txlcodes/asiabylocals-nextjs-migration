...
export default function SupplierDashboard({ supplier, onLogout }: SupplierDashboardProps) {
  ...
  const fetchSupplierStatus = async () => {
    ...
    try {
      ...
      console.log('   Environment VITE_API_URL:', process.env.NEXT_PUBLIC_API_URL || 'not set');
      ...
      try {
        ...
        throw new Error(`Backend server not accessible: ${healthError instanceof Error ? healthError.message : String(healthError)}`);
      }
  ...
  const handleSubmitTour = async (tourId: string) => {
    try {
      ...
      alert(`Failed to submit tour: ${error instanceof Error ? error.message : 'Please try again.'}`);
    }
  };
