...
export default function SupplierRegistration({ language = 'en', onClose }: SupplierRegistrationProps) {
  ...
  const submitRegistration = async () => {
    ...
    makeRequest()
      ...
      .catch((error: unknown) => {
        ...
        if (err.message?.includes('PrismaClientKnownRequestError') || err.message?.includes('P2002')) {
        ...
        // Server errors
        else if (err.message?.includes('Registration failed') || err.message?.includes('Internal server error')) {
